...
import { BookUser, Search, Download, ChevronDown, ChevronUp, FileText } from 'lucide-react';
...
const DirectoryPage = () => {
  ...
  // Deduplicate by phone, keep latest + merge info
  ...
  const filtered = useMemo(() => {
    ...
    return directory.filter(e =>
      e.latest.name.toLowerCase().includes(q) ||
      e.latest.phone.includes(q) ||
      e.latest.flatNumber.toLowerCase().includes(q) ||
      (e.latest.vehicleNumber && e.latest.vehicleNumber.toLowerCase().includes(q))
    );
  }, [directory, search]);
  ...
  const exportPDF = () => {
    ...
    const rows = filtered.map(e => {
      ...
      return `
        <tr>
          <td style="padding:8px;border-bottom:1px solid #eee;">${photoHtml}</td>
          <td style="padding:8px;border-bottom:1px solid #eee;">
            <strong>${v.name}</strong><br/>
            <span style="color:#666;font-size:12px;">${v.phone}</span>
          </td>
          <td style="padding:8px;border-bottom:1px solid #eee;">${v.flatNumber}</td>
          <td style="padding:8px;border-bottom:1px solid #eee;">${v.category}</td>
          ...
          <td style="padding:8px;border-bottom:1px solid #eee;">${format(new Date(e.firstVisit), 'dd/MM/yyyy')}</td>
        </tr>`;
    }).join('');

    const html = `<!DOCTYPE html><html><head><title>Visitor Directory</title><style>
      body{font-family:Arial,sans-serif;padding:20px}
      table{width:100%;border-collapse:collapse}
      th{text-align:left;padding:10px 8px;background:#f5f5f5;border-bottom:2px solid #ddd;font-size:13px}
      td{font-size:13px;vertical-align:middle}
      h1{font-size:20px;margin-bottom:4px}
      .sub{color:#888;font-size:13px;margin-bottom:16px}
    </style></head><body>
      <h1>Visitor Directory</h1>
      <p class="sub">Generated: ${format(new Date(), 'dd MMM yyyy, hh:mm a')} · ${filtered.length} registered visitors</p>
      <table>
        <thead><tr><th>Photo</th><th>Name / Phone</th><th>Flat</th><th>Category</th><th>Document</th><th>Visits</th><th>First Visit</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </body></html>`;
  ...
  return (
    <div className="page-container">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{directory.length} registered visitors</p>
          </div>
        </div>
      ...
      {filtered.length === 0 ? (
        <p className="text-sm text-muted-foreground text-center py-12">No visitors found</p>
      ) : (
            ...
            return (
              <div key={v.phone} className="card-section">
                <button
                  ...
                  onClick={() => setExpandedId(isExpanded ? null : v.phone)}
                >
                  {/* Avatar */}
                  ...
                  <div className="mt-3 pt-3 border-t border-border">
                    ...
                    {v.visitorPhotos.length > 0 && (
                      <div className="flex gap-2 mb-3 flex-wrap">
                        {v.visitorPhotos.map((p, i) => (
                          <img key={i} src={p} alt={`Photo ${i + 1}`} className="w-20 h-20 rounded-lg object-cover border border-border" />
                        ))}
                      </div>
                    )}
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        ...
                        <span className="ml-1 font-medium">{v.flatNumber}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Category:</span>
                        <span className="ml-1 font-medium capitalize">{v.category}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Document:</span>
                        <span className="ml-1 font-medium">{v.documentType} {v.documentNumber && `— ${v.documentNumber}`}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Purpose:</span>
                        <span className="ml-1 font-medium">{v.purpose}</span>
                      </div>
                      {v.vehicleNumber && (
                        <div>
                          <span className="text-muted-foreground">Vehicle:</span>
                          <span className="ml-1 font-mono font-medium">{v.vehicleNumber}</span>
                        </div>
                      )}
                      {v.company && (
                        <div>
                          <span className="text-muted-foreground">Company:</span>
                          <span className="ml-1 font-medium">{v.company}</span>
                        </div>
                      )}
                      <div>
                        <span className="text-muted-foreground">First visit:</span>
                        <span className="ml-1 font-medium">{format(new Date(firstVisit), 'dd MMM yyyy')}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Last visit:</span>
                        <span className="ml-1 font-medium">{format(new Date(v.entryTime), 'dd MMM yyyy')}</span>
                      </div>
                    </div>
                  </div>
                )}
