...
import { Zap, Search, UserCheck, Clock } from 'lucide-react';
...
const QuickEntryPage = () => {
  ...
  // Get unique frequent visitors (visited 2+ times), deduplicated by phone
  const frequentVisitors = useMemo(() => {
    ...
    visitors.forEach(v => {
      ...
      if (existing) {
        existing.count++;
        // Keep latest entry for most recent info
  ...
  const filtered = useMemo(() => {
    ...
    return frequentVisitors.filter(e =>
      ...
      e.visitor.phone.includes(q) ||
      e.visitor.flatNumber.toLowerCase().includes(q)
    );
    ...
    <div className="page-container">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">One-tap re-entry for frequent visitors</p>
        </div>
      </div>

      {success && (
        <div className="card-section border-success/30 mb-4 text-center">
          <p className="text-[hsl(var(--success))] text-sm font-semibold">✓ {success} logged in</p>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-muted-foreground">
            {search ? 'No matching frequent visitors' : 'No frequent visitors yet. Visitors with 2+ entries will appear here.'}
          </p>
        </div>
      ) : (
            ...
            <div key={v.phone} className="card-section flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[10px] text-muted-foreground">Flat {v.flatNumber}</span>
                  <span className="text-[10px] text-muted-foreground">·</span>
                  <span className="text-[10px] text-primary font-medium">{count} visits</span>
                </div>
              </div>
              ...
              >
                <UserCheck className="w-3.5 h-3.5" /> Enter
              </button>
            </div>
          ))}
        </div>
