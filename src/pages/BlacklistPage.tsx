...
const BlacklistPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const entry: BlacklistEntry = {
      ...
      addedBy: currentGuard?.name || 'Unknown',
    };
    ...
    <div className="page-container">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{blacklist.length} flagged entries</p>
          </div>
        </div>
        <button
      ...
      {success && (
        <div className="card-section border-success/30 mb-4 text-center">
          <p className="text-[hsl(var(--success))] text-sm font-semibold">✓ Added to blacklist</p>
        </div>
      )}
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card-section border-destructive/30 mb-4 flex flex-col gap-3">
          <div className="flex gap-2">
            <button
              type="button"
              className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${form.type === 'visitor' ? 'bg-destructive text-destructive-foreground' : 'bg-secondary text-secondary-foreground'}`}
              onClick={() => setForm(f => ({ ...f, type: 'visitor' }))}
            >
              <User className="w-3.5 h-3.5 inline mr-1" /> Visitor
            </button>
            <button
              type="button"
              className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${form.type === 'vehicle' ? 'bg-destructive text-destructive-foreground' : 'bg-secondary text-secondary-foreground'}`}
              onClick={() => setForm(f => ({ ...f, type: 'vehicle' }))}
            >
              <Car className="w-3.5 h-3.5 inline mr-1" /> Vehicle
            </button>
          </div>
          ...
          {form.type === 'visitor' && (
            <>
              ...
              <input className="input-field font-mono" placeholder="Phone *" type="tel" maxLength={10} value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value.replace(/\D/g, '') }))} />
            </>
          )}
          ...
          <div className="flex gap-2">
            <button type="submit" className="btn-primary flex-1 text-xs">Add to Blacklist</button>
            <button type="button" onClick={() => setShowForm(false)} className="btn-secondary flex-1 text-xs">Cancel</button>
          </div>
        </form>
      )}
      ...
      <div className="relative mb-4">
        ...
        <input className="input-field pl-9" placeholder="Search blacklist..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
            ...
            <div key={e.id} className="card-section border-destructive/20 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">
                  {e.type === 'visitor' ? (e.name || e.phone) : e.vehicleNumber}
                </p>
                {e.type === 'visitor' && e.phone && (
                  <p className="text-xs font-mono text-muted-foreground">{e.phone}</p>
                )}
